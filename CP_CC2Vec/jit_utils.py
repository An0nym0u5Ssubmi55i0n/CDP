...
def mini_batches(X_added_code, X_removed_code, Y, mini_batch_size=64, seed=0, shuffled=False):
    m = Y.shape[0]  # number of training examples
    mini_batches = []
    #set seed
    # TODO check if needed np.random.seed(seed)
...
def mini_batches_update_DExtended(X_ftr, X_msg, X_code, Y, mini_batch_size=64, seed=0):
    ...
    # TODO check if needed random.seed(0)
    for k in range(0, num_complete_minibatches):
        indexes = sorted(
